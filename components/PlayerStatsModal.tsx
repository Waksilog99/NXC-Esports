                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-8">
                                                    <div className="bg-white/[0.02] border border-white/5 rounded-3xl p-6 relative overflow-hidden group">
                                                        ...
                                                        <div className="flex items-end gap-3 relative z-10">
                                                            <p className={`text-3xl font-black italic ${avgPlusMinus >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                                                                {avgPlusMinus > 0 ? '+' : ''}{avgPlusMinus.toFixed(1)}
                                                            </p>
                                                            <div className="mb-1">
                                                        ...
                                                        <p className="text-[8px] text-slate-600 font-bold uppercase tracking-widest mt-2 relative z-10">
                                                            Total +/– : <span className="text-slate-400">{totalPlusMinus > 0 ? '+' : ''}{totalPlusMinus}</span> across all matches
                                                        </p>
                                                    </div>
                                            ...
                                            <div className="bg-white/5 border border-white/5 rounded-[32px] p-8">
                                                ...
                                                <div className="max-h-[460px] overflow-y-auto custom-scrollbar pr-2">
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                        {breakdown?.agents?.map((agent: any, idx: number) => {
                                                            const totalPlusMinus = agent.kills - agent.deaths;
                                                            const avgPlusMinus = totalPlusMinus / (agent.matches || 1);
                                                            const perf = getPerformanceLabel(totalPlusMinus);
                                                            ...
                                                            return (
                                                                <div key={idx} className="bg-white/[0.03] border border-white/5 rounded-2xl p-6 flex flex-col gap-4 hover:border-amber-500/20 transition-all duration-300 group">
                                                                    ...
                                                                    <div className="grid grid-cols-4 gap-2">
                                                                        ...
                                                                        <div className="bg-white/5 rounded-xl p-2 text-center border border-amber-500/10 flex flex-col justify-center overflow-hidden">
                                                                            <p className={`text-[10px] font-black ${avgPlusMinus >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                                                                                {avgPlusMinus >= 0 ? '+' : ''}{avgPlusMinus.toFixed(1)}
                                                                            </p>
                                                                            <p className="text-[6px] text-slate-500 font-black uppercase tracking-[0.2em] -mt-0.5">+/-</p>
                                                                            <p className={`text-[5px] font-black uppercase tracking-tighter truncate ${perf.color}`}>{perf.label.replace(' Avg', '')}</p>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            );
